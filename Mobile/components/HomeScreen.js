...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    const navigateUser = (res, token) => {
      if ("user" in res) {
        navigation.navigate("MainMenu", { token: token, userData: res.user })
      } else {
  ...
  return (
    <ImageBackground source={Bg} style={{ flex: 1 }}>
      <View style={{ flex: 1, padding: 10 }}>
        <Text style={{ color: "white", padding: 15, fontSize: 48, fontWeight: "bold" }}>PickApp</Text>
      </View>
      {getToken ? (
        <>
          <View
            ...
            <Text style={{ color: "white", fontSize: 36 }}>Reduce food waste</Text>
            <Text style={{ color: "white", opacity: 0.7, fontSize: 16 }}>
              Polish households waste 2 million tons of food per year, meaning an
              estimated 54 kg per Polish citizen per year.
            </Text>
          </View>
          <View style={{ flex: 3 }}>
            <TouchableOpacity
              ...
              <Text style={{ fontSize: 20, color: "#59981A", fontWeight: "bold" }}>
                Log in
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
