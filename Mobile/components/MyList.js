...
const MyList = ({ data, token }) => {
    ...
    const validateCategories = (categories) => {
        ...
        if (categories.length == 1) {
            ...
            return (<Image style={{ width: 50, height: 50, marginLeft: 20 }} source={a} />)
        } else {
            return (
                <Image style={{ width: 50, height: 50, marginLeft: 20 }} source={require("../assets/star.png")} />
            )
        }
    ...
    if (!disabled) {
        return (
            ...
            <View style={{ display: disabled ? "none" : "initial", marginTop: 20, justifyContent: "space-between", display: "flex", alignItems: "center", flexDirection: "row", position: "relative", width: "100%", height: 68, borderRadius: 30, backgroundColor: "#ECF87F", paddingRight: 20 }}>
                {validateCategories(data.categories)}
                <Text style={{ marginLeft: 5, width: 80 }}>{data.categories.split(",").length == 1 ? data.categories : "Many"}</Text>
                <Text style={{ color: status ? "#59981A" : "#D7263D", marginLeft: 15, fontSize: 15, width: 80, fontWeight: "bold" }}>{status ? "Avaible" : "Reserved"}</Text>
                <Switch
                    ...
                    value={status}
                />
                <TouchableOpacity onLongPress={() => showAlert(data.id)} style={{ position: "absolute", top: -10, left: 0 }}><Image tintColor="#f4f3f4" style={{ width: 30, height: 30 }} source={X} /></TouchableOpacity>
            </View>
        )
    }
